...
| `gpt-5.6-sol` | none, low, medium, high, xhigh, max, ultra | no minimal; ultra is advertised in the Codex model catalog |
| `gpt-5.6-terra` | none, low, medium, high, xhigh, max, ultra | no minimal; ultra is advertised in the Codex model catalog |
...
The cybersecurity models (`gpt-5.6-cyber`, `gpt-daybreak-red-latest`, `gpt-daybreak-blue-latest`) are deliberately unregistered: the API models page offers them only for authorized security work, so a policy could otherwise assign a model most operators cannot call. `codex-auto-review` is internal and outside this namespace's `match`.
...
| `claude-opus-5-5` | low, medium, high, xhigh, max | current Opus lineup, released 2026-09-22; the dateless ID is its own pinned snapshot; on both the xhigh and max lists; default effort medium, unlike every other effort-capable model |
| `claude-opus-5` | low, medium, high, xhigh, max | legacy but still available since Opus 5.5 shipped; supports all five effort levels; default effort high |
...
`claude-mythos-5` and `claude-mythos-5-1` are deliberately unregistered: they are invite-only (Project Glasswing) per [platform.claude.com/docs/en/models/mythos-5-1/overview](https://platform.claude.com/docs/en/models/mythos-5-1/overview), so a policy could otherwise assign a model most operators cannot call.
