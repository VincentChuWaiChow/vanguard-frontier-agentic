...
|---|---|---|
| codex → OpenAI | HTTP 404, `error.code: "model_not_found"`, `error.type: "invalid_request_error"` | Codex sends the configured effort unchanged (only `ultra` and `persistent` are rewritten), so a pairing the model does not support is accepted or rejected by the API; the error shape for that case is not verified here |
