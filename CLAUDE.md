...
- **Delegate by default.** Follow `.claude/skills/agentic-delegation/SKILL.md`: Haiku or Sonnet subagents for read-only exploration, research, and CI-log reading (require file:line or URL citations, and name the model on every call — built-in Explore otherwise inherits the session model), Sonnet subagents for prose against an exact file-scoped spec, code edits on Opus 5.5, and the orchestrator keeps architecture decisions, security-sensitive edits, verification, and the commit. Start at Opus 5.5's default `medium` effort and escalate on evidence; the skill holds the ladder. A delegate's self-report is not verification — read the diff and run the gates yourself.
- **Run it as a pipeline when the work is big.** `.claude/workflows/agentic-delegation.js` is that skill as an executable `Workflow` — Haiku recon, a Context7 verification phase, Opus implementers against file-scoped specs (Sonnet for Markdown-only specs), an independent refuter per change, then the gate suite. Invoke as `Workflow({name: "agentic-delegation", args: {task, questions, libraries, specs}})`; see `.claude/workflows/README.md`. It never commits — the orchestrator still reads the diff and commits.
...
- **Orchestrator requirements.** Haiku must never be the orchestrator. Opus 5.5 is the default orchestrator. When Sonnet orchestrates instead, run it at high reasoning effort at minimum (use the harness's maximum-thinking mode if available) — planning and delegation quality degrade below that, and a weak plan wastes every delegate downstream.
